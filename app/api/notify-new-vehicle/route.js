...
export async function POST(request) {
  try {
    ...
    const { make, model, title, year, creatorEmail, vehicleId } = await request.json()

    const bikeName = [make, model, title ? `— ${title}` : null].filter(Boolean).join(' ')
    const vehicleUrl = vehicleId
      ? `https://zweitakthoden.de/vehicles/${vehicleId}`
      : 'https://zweitakthoden.de/vehicles'

    await transporter.sendMail({
      from: `"Zweitakthoden" <${process.env.NOTIFY_SMTP_USER}>`,
      to: process.env.NOTIFY_SMTP_USER,
      subject: `Neues Bike: ${bikeName}`,
      html: `<!DOCTYPE html>
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:18px;border:1.5px solid #1a1108;box-shadow:5px 5px 0 #1a1108;overflow:hidden;">
        <tr>
          <td style="background:#1a1108;padding:24px 32px;">
            ...
            <p style="margin:8px 0 0;font-family:Arial,system-ui,sans-serif;font-size:30px;color:#ffffff;line-height:1.15;font-weight:800;">Neues <em>Bike.</em></p>
          </td>
        </tr>
        <tr>
          <td style="background:#ffffff;padding:36px 32px 32px;">
            <p style="margin:0 0 10px;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#5e5248;font-family:ui-monospace,'Courier New',monospace;">Fahrzeug</p>
            <p style="margin:0 0 6px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Bike:</strong> ${bikeName}</p>
            ${year ? `<p style="margin:0 0 6px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Baujahr:</strong> ${year}</p>` : ''}
            <p style="margin:0 0 24px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Eingestellt von:</strong> ${creatorEmail}</p>
            ...
            <table cellpadding="0" cellspacing="0"><tr>
              <td style="background:#1a1108;border-radius:100px;">
                <a href="${vehicleUrl}" style="display:inline-block;padding:13px 28px;font-family:Arial,sans-serif;font-size:12px;letter-spacing:1.5px;text-transform:uppercase;color:#ffffff;text-decoration:none;">
                  Bike ansehen →
                </a>
              </td>
            </tr></table>

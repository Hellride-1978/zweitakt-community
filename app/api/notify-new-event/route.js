...
export async function POST(request) {
  try {
    ...
    const eventUrl = eventId
      ? `https://zweitakthoden.de/events/${eventId}`
      : 'https://zweitakthoden.de/events'
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:18px;border:1.5px solid #1a1108;box-shadow:5px 5px 0 #1a1108;overflow:hidden;">
        <tr>
          <td style="background:#1a1108;padding:24px 32px;">
            ...
            <p style="margin:8px 0 0;font-family:Arial,system-ui,sans-serif;font-size:30px;color:#ffffff;line-height:1.15;font-weight:800;">Neuer <em>Termin.</em></p>
          </td>
        </tr>
        <tr>
          <td style="background:#ffffff;padding:36px 32px 32px;">
            <p style="margin:0 0 10px;font-size:10px;letter-spacing:2.5px;text-transform:uppercase;color:#5e5248;font-family:ui-monospace,'Courier New',monospace;">Ausfahrt / Treffen</p>
            <p style="margin:0 0 6px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Titel:</strong> ${title}</p>
            ${date ? `<p style="margin:0 0 6px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Datum:</strong> ${date}</p>` : ''}
            ${location ? `<p style="margin:0 0 6px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Ort:</strong> ${location}</p>` : ''}
            <p style="margin:0 0 24px;font-size:15px;color:#1a1108;font-family:Arial,system-ui,sans-serif;"><strong>Erstellt von:</strong> ${creatorEmail}</p>

...
export default function NewVehiclePage() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('vehicles')
        .insert({ ...basePayload, ...urlUpdates })

      if (insertError) {
        const isMissingCol = insertError.message?.includes('image_url_2') ||
          insertError.message?.includes('image_url_3') ||
          insertError.message?.includes('image_url_4')
        if (isMissingCol) {
  ...
  if (loading) return (
    <DesktopLayout crumb="Neues Fahrzeug">

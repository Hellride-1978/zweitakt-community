    ...
    <DesktopLayout crumb={`${vehicle.make} ${vehicle.model}`}>
      <div className="bike-detail-grid">
        ...
        <div style={{ minWidth: 0, paddingRight: 4, paddingBottom: 4 }}>
          ...
          {vehicle.title && (
            <div className="zd-mono accent" style={{ marginBottom: 32 }}>· {vehicle.title}</div>
          )}

          {/* Spec grid — nur rendern wenn mind. ein Wert vorhanden */}
          {(vehicle.year || vehicle.displacement_cc) && (
            <div className="spec-grid-d">
              {vehicle.year && (
          ...
          {vehicle.description && (
            <>
              <div className="zd-mono accent" style={{ marginBottom: 14 }}>Beschreibung</div>
              <div className="zd-card" style={{ marginBottom: 32 }}>
